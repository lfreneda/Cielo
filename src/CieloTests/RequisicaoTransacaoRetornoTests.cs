...
{
    [TestFixture]
    public class RequisicaoTransacaoRetornoTests
    {
        ...
        [Test]
        public void GivenAnReponse_CanGetTid()
        {
            var retorno = new RequisicaoTransacacaoRetorno(ExpectedResponse);
            retorno.Tid.Should().Be("100699306908642F1001");
        }

        [Test]
        public void GivenAnReponse_CanGetUrlAuthentication()
        {
            var retorno = new RequisicaoTransacacaoRetorno(ExpectedResponse);
            retorno.UrlAutenticacao.Should().Be("https://ecommerce.cielo.com.br/web/index.cbmp?id=a783251");
        }
    }
}
